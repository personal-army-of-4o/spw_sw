...
#define CONTROL_TX(pn, cw)                                                     \
    control[(pn)] = cw // set control word to cw for port pn
...
#define GOT_PKG(pn)                                                            \
    (regs[*port_num * (pn) + REQUEST_OFFSET] >> REQUEST_INDEX) & 0x1
#define TADDR(pn)                                                              \
    (regs[*port_num * (pn) + RTADDR_OFFSET] >> TADDR_INDEX) & TADDR_MASK
#define CONTROL_TX(pn, cw)                                                     \
    regs[*port_num * (pn) + RCONTROL_TX_OFFSET] |= (cw & CONTROL_TX_MASK)      \
                                                   << CONTROL_TX_INDEX
#define GRANT(pn) regs[*port_num * (pn) + RGRANT_OFFSET] |= 0x1 & GRANT_INDEX
#define DISCARD(pn)                                                            \
    regs[*port_num * (pn) + RDISCARD_OFFSET] |= 0x1 & DISCARD_INDEX
#define AVAILABLE(pn)                                                          \
    (regs[*port_num * (pn) + RAVAILABLE_OFFSET] >> AVAILABLE_INDEX) & 0x1
...
volatile Table *table =
    <define value somehow>; // make linter place this variable in separate
                            // region after all others

// these constants are unused in this software, but will be used by remote
// update software
volatile uint32_t *table_size_addr = 0;
volatile uint32_t *table_addr = 1;
volatile uint32_t *table_region_max_range =
    2; // make linter populate this as OCRAM_SIZE-<size of all regions except
       // table region>
